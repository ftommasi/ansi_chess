...
pub async fn main() -> std::io::Result<()> {
    ...
    select! {
        white_move = white_listener.accept() =>{
            /* got client input */
            process_client_input(white_move.unwrap().0).await;
            bcast_to_clients().await;
        },
        black_move = black_listener.accept() =>{
            /* got client input */
            process_client_input(black_move.unwrap().0).await;
            bcast_to_clients().await;
        },
    }
    Ok(())
