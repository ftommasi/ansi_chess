...
fn test_print() {
    let chess_board = ansi_chess::Board::new();
...
fn interactive_mode() {
    ...
    for piece in &mut chess_board.pieces {
        let mut moves = ansi_chess::get_valid_moves_for_piece(&piece.clone(), &chess_board_clone); //TODO:
                                                                                                   //refactor
                                                                                                   //into
                                                                                                   //lib
    ...
    'game: loop {
        ...
        for (idx_r, rank) in (1..9).rev().enumerate() {
            ...
            for (idx_c, file) in (1..9).enumerate() {
                ...
                if ansi_chess::square_contains_piece_square(
                    ...
                    let cur_piece = ansi_chess::get_piece_at(
                        &chess_board,
                        ansi_chess::SquareRank::from(rank),
                        ansi_chess::SquareFile::from(file),
                    )
        ...
        if input_buffer.contains('X') {
            ...
            let mut test_path_out = std::env::current_dir().unwrap_or_default();
            let mut test_path_in = std::env::current_dir().unwrap_or_default();
                    ...
                    if ansi_chess::square_contains_piece_square(
                        ...
                        if let Some(piece_id) = ansi_chess::get_piece_id_at(
                            ...
                            for piece in &mut chess_board.pieces {
                                ...
                                let mut moves = ansi_chess::get_valid_moves_for_piece(
                                    &piece.clone(),
                                    &chess_board_clone,
                                );
                                piece.possible_moves.append(&mut moves);
